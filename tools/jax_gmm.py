...
def e_step(X, pi, mu, sigma):
    mixture_log_prob = jaxd.MultivariateNormalTriL(
        loc=mu, scale_tril=jnp.linalg.cholesky(sigma)).log_prob(
            X[:, None, ...]) + jnp.log(pi)
...
def m_step(X, membership_weight):
    ...
    mu_updated = jnp.sum(X[:, None, ...] * membership_weight[..., None],
...
def compute_vlb(X, pi, mu, sigma, membership_weight):
    component_log_prob = jaxd.MultivariateNormalTriL(
        loc=mu, scale_tril=jnp.linalg.cholesky(sigma)).log_prob(X[:, None,
                                                                  ...])
    vlb = membership_weight * (jnp.log(pi) + component_log_prob - jnp.log(
        jnp.clip(membership_weight, a_min=jnp.finfo(np.float32).eps)))
    return jnp.sum(vlb)


@partial(jax.jit, static_argnums=(1, ))
def train_em_jax(observed,
                 num_comp,
                 n_init=25,
                 rtol=1e-3,
                 max_iter=100,
    ...
    def one_step(state):
        ...
        pi_updated, mu_updated, sigma_updated = m_step(observed,
                                                       membership_weight)
        loss_updated = compute_vlb(observed, pi_updated, mu_updated,
                                   sigma_updated, membership_weight)
        loss_diff = jnp.abs((loss_updated / loss) - 1.)
        return (i + 1, (pi_updated, mu_updated, sigma_updated), loss_updated,
                loss_diff)
    ...
    init_val = (jnp.zeros([n_init], jnp.int32), (pi_init, mu_init, sigma_init),
                -jnp.ones([n_init]) * jnp.inf, jnp.ones([n_init]) * jnp.inf)
    num_iter, (pi_est, mu_est,
               sigma_est), loss, loss_diff = jax.lax.while_loop(
                   cond_fn, one_step, init_val)
    index = jnp.argmax(loss)
    pi_best, mu_best, sigma_best = jax.tree_map(lambda x: x[index],
                                                (pi_est, mu_est, sigma_est))
...
def _log_likelihood(X, pi, mu, sigma):
    mixture_log_prob = jnp.log(pi) + jaxd.MultivariateNormalTriL(
        loc=mu, scale_tril=jnp.linalg.cholesky(sigma)).log_prob(X[None, ...])
    return jsp.special.logsumexp(mixture_log_prob, axis=-1, keepdims=True)[0]


log_likelihood = jax.jit(
    jax.vmap(_log_likelihood, in_axes=(0, None, None, None)))

d_log_likelihood = jax.jit(
    jax.vmap(jax.grad(_log_likelihood, argnums=0),
...
likelihood = jax.jit(jax.vmap(_likelihood, in_axes=(0, None, None, None)))

d_likelihood = jax.jit(
    jax.vmap(jax.grad(_likelihood, argnums=0), in_axes=(0, None, None, None)))
